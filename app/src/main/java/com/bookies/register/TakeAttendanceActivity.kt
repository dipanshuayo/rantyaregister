...
class TakeAttendanceActivity : AppCompatActivity() {
    ...
    private fun createStudentAttendance() {
        studentAttendanceFragment =
            StudentAttendanceFragment.newInstance(studentsNameArray, studentsAttendance, "SAVE")
        supportFragmentManager
            .beginTransaction()
            .add(
                R.id.student_attendance_fragment_holder,
                if (state getBooleanValue "isStudentNameAdded") studentAttendanceFragment else AddStudentsFragment.newInstance(
                    arrayOf()
                ),
                "studentAttendance"
            )
